...
def conc_model(conn, user_name: str = "Lachlan"):
    ...
    while True:
        ...
        styrene_valid = all(
            r2 >= 0.90 and mse < 0.001 for r2, mse in zip(metrics['R² Styrene'], metrics['MSE Styrene']))
        polystyrene_valid = all(
            r2 >= 0.90 and mse < 0.001 for r2, mse in zip(metrics['R² Polystyrene'], metrics['MSE Polystyrene']))
...
def conc_model_for_testing(conn, user_name: str = "Lachlan"):
    ...
    while True:
        ...
        # Decision based on metrics for verification
        styrene_valid = all(
            r2 >= 0.80 and mse < 0.005 for r2, mse in zip(metrics['R² Styrene'], metrics['MSE Styrene']))
        polystyrene_valid = all(
            r2 >= 0.80 and mse < 0.005 for r2, mse in zip(metrics['R² Polystyrene'], metrics['MSE Polystyrene']))
...
class Experiment:
    def __init__(self, user_name: str, conn):
        self.user_name = user_name
        self.conn = conn
        self.start_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
        self.out_path = get_output_path()
    ...
    def save_metadata(self):
        ...
        log_msg(f"Experiment ended at {end_time}")
        metadata_path = os.path.join(self.out_path, f'experiment_metadata {end_time.replace(":", "")}.json')
        with open(metadata_path, 'w', encoding='utf-8') as f:
            json.dump(self.experiment_metadata, f, indent=4, ensure_ascii=False)
        log_msg("Metadata saved")
...
class PlateReader:
    ...
    def take_background(self):
        ...
        log_msg("Requesting plate background from reader")
        send_message(self.conn, "PLATE_BACKGROUND", "Empty Plate Reading")
        msg_type, msg_data = receive_message(self.conn)
        if msg_type == "PLATE_BACKGROUND":
            log_msg("Plate background data received")
            return msg_data

    def take_measurement(self):
        send_message(self.conn, "RUN_PROTOCOL", "Empty Plate Reading")
        msg_type, msg_data = receive_message(self.conn)
        if msg_type == "CSV_FILE":
            return msg_data
    ...
    def check_stable_temp(self):
        pass


class OT2Robot:
    def __init__(self):
        self.protocol_path = get_file_path()
        self.protocol_name = os.path.basename(self.protocol_path)
    ...
    def run_ssh_command(self):
        ...
        try:
            ...
            username = "root"  # OT-2 default username is 'root'
            key_path = r"C:\Users\Lachlan Alexander\ot2_ssh_key"  # Path to your SSH private key
            protocol_path = f"'/data/user_storage/prd_protocols/{self.protocol_name}'"  # Path to your protocol on the OT-2
    ...
    def upload_protocol(self):
        self.upload_file(self.protocol_path, "Uploading protocol to OT-2")

    def upload_file(self, file_path, message):
        while True:
            try:
                log_msg(message)
                self.run_subprocess(file_path)
                log_msg("Upload complete")
                break
            except Exception as e:
                log_msg(f"Error uploading: {e}")
                if input(">>> Retry upload? (yes/no): \n>>> ").lower() != "yes":
                    log_msg("Upload cancelled by user")
                    break

    def run_protocol(self):
        output = self.run_ssh_command()  # Replace with actual SSH command
        while not output:
            if input(">>> Proceed manually? (yes/no): \n>>> ").lower() == "yes":
                break
...
class ModelSet:
    ...
    def run_screening(self, background_path, data_path, volumes_df):
        log_msg("Running ML screening")
        self.models, self.metrics, self.scaler = ml_screening(background_path, data_path, volumes_df, self.out_path)
        return self.metrics

    def verify_models(self, background_path, data_path, volumes_df):
        log_msg("Verifying ML models")
        verify_models(background_path, data_path, volumes_df, self.out_path, self.models, self.scaler)


class ExperimentRunner:
    def __init__(self, conn, user_name="Lachlan"):
        self.conn = conn
        self.user_name = user_name
        self.experiment = Experiment(user_name, conn)
        self.plate_reader = PlateReader(conn)
        self.robot = OT2Robot()
        self.ml_model = ModelSet(self.experiment.out_path)

    def run_concentration_experiment(self):
        background_path = self.plate_reader.take_background()

        volumes_df, volumes_path = gen_volumes_csv(self.experiment.out_path)

        # self.robot.upload_file(volumes_path, "Uploading volumes file")
        log_msg("Upload volumes")
        # self.robot.upload_protocol()
        log_msg("Upload protocol")
        # self.robot.run_protocol()
        log_msg("Run protocol")

        data_path = self.plate_reader.take_measurement()

        metrics = self.ml_model.run_screening(background_path, data_path, volumes_df)

        self.evaluate_and_verify(metrics)

        self.experiment.save_metadata()

    def evaluate_and_verify(self, metrics):
        styrene_valid = all(
            r2 >= 0.80 and mse < 0.005 for r2, mse in zip(metrics['R² Styrene'], metrics['MSE Styrene']))
        polystyrene_valid = all(
            r2 >= 0.80 and mse < 0.005 for r2, mse in zip(metrics['R² Polystyrene'], metrics['MSE Polystyrene']))
        ...
        if styrene_valid or polystyrene_valid:
            log_msg("Model parameters good, preparing verification samples.")
            background_path = self.plate_reader.take_background()
            ...
            self.ml_model.verify_models(background_path, data_path, volumes_df)

        else:
            log_msg("Model parameters poor. Closing loop.")
            return False

    def run_time_measurements(self, interval_minutes=10, num_measurements=7):
        background_path = self.plate_reader.take_background()
        time_stamps, data_paths = [], []

        for _ in range(num_measurements):
            data_path = self.plate_reader.take_measurement()
            data_paths.append(data_path)
            time_stamps.append(datetime.now())
            time.sleep(interval_minutes * 60)  # Interval between measurements

        self.plot_results_over_time(data_paths, background_path, time_stamps)

    def plot_results_over_time(self, data_paths, background_path, time_stamps):
        averages, std_devs = [], []
        for path in data_paths:
            plate = load_data_new(background_path)
            data = load_data_new(path)
            corrected_array = separate_subtract_and_recombine(data, plate, 0).iloc[:25, 40].to_numpy()
            averages.append(np.average(corrected_array))
            std_devs.append(np.std(corrected_array))
        ...
        ax.grid(True)
        plt.tight_layout()
        plt.savefig(os.path.join(self.experiment.out_path, "absorbance_over_time.png"))
        plt.show()
...
def handle_client(conn):
    ...
    try:
        ...
        while True:
            ...
            if choice == "1":
                runner = ExperimentRunner(conn, user_name)
                runner.run_concentration_experiment()
