...
def get_csv():
    ...
    data_directory = r'C:\Users\Lachlan Alexander\Desktop\Uni\2024 - Honours\Experiments'
...
def load_csv_to_dataframe(csv_file: str):
    """
    Load a CSV file into a Pandas DataFrame.

    This function attempts to read the given CSV file and load it into a Pandas DataFrame for further analysis.

    :param csv_file: The file path to the CSV file.
    :raises Exception: If the CSV file cannot be loaded.
    :return: Pandas DataFrame containing the data from the CSV file.
    """
    try:
        # Load the CSV file into a Pandas DataFrame
        df = pd.read_csv(csv_file)
        log_msg(f"Loaded file: {csv_file}")
        return df
    except Exception as e:
        log_msg(f"Failed to load CSV file: {e}")
        raise
...
def load_data_new(path: str, start_wavelength: int = 220, end_wavelength: int = 1000) -> pd.DataFrame:
    ...
    Best used for loading CSV files obtained from SPECTROstar Nano protocols.
    ...
    try:
        ...
        while True:
            ...
            if msg_type == "NEXT_READING":
                ...
                # Send data CSV path
                data = get_csv()
                log_msg(load_data_new(data))
                send_message(s, "SAMPLE_DATA_COLLECTED", data)
